...
public interface SanPhamRepository extends JpaRepository<SanPham, String> {
    ...
    @Query(value = """
                ...
                WHERE b.deleted = 1
                AND (a.gia_ban >= :toPrice AND a.gia_ban <= :fromPrice)
    ...
    @Query(value = "SELECT sp FROM SanPham sp WHERE sp.deleted = 1 ORDER BY sp.ngayTao DESC")
    List<SanPham> getAll();
    ...
    @Query(value = "SELECT b.ma AS maSanPham, b.ten AS tenSanPham, SUM(a.so_luong_ton) AS soLuongTon, b.deleted AS status\n" +
            "FROM san_pham_chi_tiet a\n" +
            "JOIN san_pham b ON a.id_san_pham = b.id\n" +
            "WHERE a.deleted = 1 OR b.deleted = :status OR b.ma LIKE '%:ma%' OR b.ten LIKE '%:ten%'\n" +
    ...
    @Query(value = "SELECT b.ma AS maSanPham, b.ten AS tenSanPham, SUM(a.so_luong_ton) AS soLuongTon, b.deleted AS status\n" +
            "FROM san_pham_chi_tiet a\n" +
            "JOIN san_pham b ON a.id_san_pham = b.id\n" +
            "WHERE a.deleted = 1 OR b.deleted = 1 OR b.deleted = 0 OR b.ma LIKE '%:ma%' OR b.ten LIKE '%:ten%'\n" +
            "GROUP BY b.ma, b.ten, b.deleted\n" +
            "HAVING soLuongTon >= :soLuongTon - 10 AND soLuongTon <= :soLuongTon + 10\n" +
            "ORDER BY MAX(b.ngay_tao) DESC\n" +
            "LIMIT 0, 1000;", nativeQuery = true)
